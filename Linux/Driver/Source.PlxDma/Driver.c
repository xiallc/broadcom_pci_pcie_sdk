    ...
    )
{
    ...
    // Check for interrupt resource & install ISR
    if (pdx->pPciDevice->irq != 0)
    {
        DebugPrintf(("Attempt to enable MSI interrupt...\n"));
        ...
        if (pdx->IrqType == PLX_IRQ_TYPE_MSI)
        {
            ...
            if (pdx->OffsetCap_MSI != 0)
            {
                DebugPrintf((
                    "     MSI Addr : %08X_%08X\n",
                    PLX_DMA_REG_READ( pdx, pdx->OffsetCap_MSI + 0x8),
                    PLX_DMA_REG_READ( pdx, pdx->OffsetCap_MSI + 0x4)
                    ));
                DebugPrintf((
                    "     MSI Data : %04X\n",
                    PLX_DMA_REG_READ( pdx, pdx->OffsetCap_MSI + 0xC)
                    ));
            }
        }
