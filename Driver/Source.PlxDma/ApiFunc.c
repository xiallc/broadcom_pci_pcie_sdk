    ...
    )
{
    ...
    if ((pdx->Key.PlxChip & 0xFF00) == 0x8600)
    {
    ...
    )
{
    ...
    // Set Transfer Count, valid bit [31], & enable done interrupt[30] if requested
    RegValue = pParams->ByteCount | (1 << 31);
    ...
    )
{
    ...
    // Get DMA interrupt control/status
    RegValue = PLX_DMA_REG_READ( pdx, OffsetDmaBase + 0x3C );

    // Disable invalid descriptor interrupt ([1])
    RegValue &= ~(1 << 1);
