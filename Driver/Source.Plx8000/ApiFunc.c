    ...
    )
{
    ...
    *pFlag = FALSE;
    ...
    PlxChip = pdx->Key.PlxChip & 0xFF00;

    if ((PlxChip != 0x8600) && (PlxChip != 0x8700))
        PlxChip = pdx->Key.PlxChip;
    ...
    )
{
    ...
    *pReqId = 0;
    ...
    // 8500 series NT-Virtual side not supported
    if (((pdx->Key.PlxChip & 0xFF00) == 0x8500) &&
         (pdx->Key.NTPortType == PLX_NT_PORT_VIRTUAL))
    {
        DebugPrintf(("ERROR - 8500 series NT-Virtual side ReqID not yet supported\n"));
        return ApiUnsupportedFunction;
    }
    ...
    // Clear any AER UR errors [20]
    PLX_PCI_REG_WRITE( pdx, (U16)(Offset_CapAer + 0x4), (1 << 20) );

    // Make sure AER UR error is not masked [20]
    PLX_PCI_REG_READ( pdx, (U16)(Offset_CapAer + 0x8), &RegAerMask );
    if (RegAerMask & (1 << 20))
        PLX_PCI_REG_WRITE( pdx, (U16)(Offset_CapAer + 0x8), RegAerMask & ~(1 << 20) );
    ...
    /*********************************************************
     * Issue a TLP read or write to a PLX register
     ********************************************************/
    if (bReadTlp)
        PlxRegisterRead( pdx, 0, NULL, TRUE );
    else
        PlxRegisterWrite( pdx, 0, WriteValue, TRUE );

    // Restore PCI command/status
    ...
    {
        DebugPrintf((
            "WARNING - Error Pointer reports bit %d not UR (20). Log may be invalid\n",
            (RegValue & 0x1F) 
            ));
    }
    ...
    )
{
    ...
    if (IndexToUse == (U16)-1)
    {
        for (index=0; index <= MaxIndex; index++)
        {
            ...
            if (LutWidth == sizeof(U32))
                offset = BaseOffset + (index * sizeof(U32));
            else
                offset = BaseOffset + ((index & ~(1 << 0)) * sizeof(U32));
    ...
    *pLutIndex = IndexToUse;
    ...
    if (LutWidth == sizeof(U32))
        ...
        offset = BaseOffset + ((IndexToUse & ~(1 << 0)) * sizeof(U32));
