    ...
    )
{
    ...
    // Make sure the PCI class is "Base System Peripheral"
    ...
    )
{
    ...
    *pSglAddress = 0;
    ...
    VaSgl  = (VaSgl + (64 - 1)) & ~(64 - 1);
    BusSgl = (BusSgl + (64 - 1)) & ~(64 - 1);
    ...
    for (i = 0; i < TotalDescr; i++)
    {
        ...
        *(U32*)(VaSgl + 0x0) = TmpValue;
        ...
        if (BytesRemaining != 0)
        {
            ...
            // Adjust PCI address unless should remain constant
            if ((pDma->bConstAddrSrc == FALSE) && (pDma->bConstAddrDest == FALSE))
                PciAddr += BlockSize;
